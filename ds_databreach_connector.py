...
class DSDataBreachConnector:
    ...
    def get_data_breach(self, param):
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Error Connecting to server. {error_message}")
        if breach_total > 0:
            summary = {"data_breach_count": breach_total, "data_breach_found": True}
            action_result.update_summary(summary)
    ...
    def get_data_breach_record(self, param):
        ...
        if breach_record_total > 0:
            summary = {"data_breach_record_count": breach_record_total, "data_breach_record_found": True}
            action_result.update_summary(summary)
            ...
            summary = {"data_breach_record_count": 0, "data_breach_record_found": False}
            action_result.update_summary(summary)
    ...
    def get_data_breach_record_by_username(self, param):
        ...
        try:
            breach_record_view = DataBreachRecordService.data_breach_records_view(
                username=user_name, published=published_date_range, domain_names=domain_names_param, review_statuses=review_statuses_param
            )
            self._connector.save_progress(f"Breach record View: {breach_record_view}")
            breach_record_pages = breach_record_service.read_all_records(view=breach_record_view)
        except StopIteration:
            error_message = "No DataBreach objects retrieved from the Digital Shadows API in page groups"
            return action_result.set_status(phantom.APP_ERROR, f"Error Details: {error_message}")
    ...
    def get_data_breach_record_reviews(self, param):
        ...
        if breach_record_reviews_total > 0:
            summary = {"breach_record_reviews_count": breach_record_reviews_total, "breach_record_reviews_found": True}
            action_result.update_summary(summary)
            for breach_record_review in breach_record_reviews:
                action_result.add_data(breach_record_review)
            action_result.set_status(
                phantom.APP_SUCCESS, f"Digital Shadows breach record reviews fetched for the Breach Record ID: {breach_record_id}"
            )
        return action_result.get_status()
    ...
    def post_breach_record_review(self, param):
        ...
        try:
            ...
            error_message = self._handle_exception_object.get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Error Connecting to server. {error_message}")
        summary = {"breach_record_reviews_status_code": response["status"], "breach_record_reviews_message": response["message"]}
        action_result.update_summary(summary)
