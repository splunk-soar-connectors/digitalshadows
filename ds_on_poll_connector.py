...
class DSOnPollConnector:
    ...
    def on_poll(self, param):
        ...
        if start_time is None or end_time is None:
            ...
            ret_val, self._history_days_interval = self._handle_exception_object.validate_integer(
                action_result, self._history_days_interval, HISTORY_DAYS_INTERVAL_KEY
            )
            if phantom.is_fail(ret_val):
                return action_result.get_status()
            date_range = f"P{self._history_days_interval}D"
            ...
            if self._private_incident:
                try:
                    ...
                    incident_view = IncidentService.incidents_view(
                        date_range=date_range, date_range_field="published", statuses=["READ", "UNREAD"], types=incident_types
                    )
                    self._connector.save_progress(f"incident req view: {json.dumps(incident_view, ensure_ascii=False)}")
                ...
                for incident_page in incident_pages:
                    for incident in incident_page:
                        ...
                        if status == phantom.APP_SUCCESS:
                            ...
                            self._connector.error_print(f"Did not ingest incident {incident.id}")
                ...
                if j != incident_total:
                    action_result.set_status(phantom.APP_ERROR, status_message="Did not receive all the incident from Digital Shadows")
                else:
            ...
            if self._global_incident:
                try:
                    ...
                    intelligence_incident_view = IntelligenceIncidentService.intelligence_incidents_view(
                        date_range=date_range, date_range_field="published", types=incident_types
                    )
                    self._connector.save_progress(f"intelligence_incident_view: {json.dumps(intelligence_incident_view, ensure_ascii=False)}")
                except Exception as e:
                    error_message = self._handle_exception_object.get_error_message_from_exception(e)
                    return action_result.set_status(phantom.APP_ERROR, f"{SERVICE_ERROR_MESSAGE} {error_message}")
                ...
                for intelligence_incident_page in intelligence_incident_pages:
                    for intelligence_incident in intelligence_incident_page:
                        self._connector.save_progress(f"count: {k}")
                        status, message = self._save_intel_incident(intelligence_incident)
                        if status == phantom.APP_SUCCESS:
                            k += 1
                            self._connector.save_progress(
                                DS_POLL_INCIDENT_COMPLETE.format(intelligence_incident.id, k, intelligence_incident_total)
                            )
                        else:
                            self._connector.error_print(f"Did not ingest intel-incident {intelligence_incident.id}")
                            action_result.set_status(phantom.APP_ERROR, message)
                            self._connector.add_action_result(action_result)
                            return action_result.get_status()
                ...
                if k != intelligence_incident_total:
                    action_result.set_status(
                        phantom.APP_ERROR, status_message="Did not receive all the intelligence incident from Digital Shadows"
                    )
                else:
                    action_result.set_status(phantom.APP_SUCCESS)
    ...
    def _save_data_breach(self, breach):
        ...
        if status == phantom.APP_SUCCESS:
            ...
            for breach_record_page in breach_record_service.find_all_pages(breach.id):
                artifacts = list(
                    map(
                        lambda breach_record: self._prepare_artifact(container_id, container["severity"], breach, breach_record),
                        breach_record_page,
                    )
                )
    ...
    def _prepare_intel_incident_container(self, intelligence_incident):
        ...
        container["name"] = "{} - {}".format(
            intelligence_incident.payload["type"].title().replace("_", " "), anyascii(intelligence_incident.payload["title"])
        )
        ...
        container["description"] = f"{intel_incident_desc}"
    ...
    def _prepare_incident_container(self, incident):
        ...
        container["description"] = f"{incident_desc}"
        container["custom_fields"] = dict()
    ...
    def _prepare_container(self, breach):
        ...
        container = dict()
        container["label"] = self._container_label
        container["name"] = f"Digital Shadows Data Breach {breach.id}"
        container["description"] = f"{breach.incident_title}"
    ...
    def _prepare_artifact(self, container_id, container_severity, breach, breach_record):
        ...
        link_to_incident = f"{ds_api_host}/client/incidents/{breach.incident_id}"

        artifact = dict()
        artifact["container_id"] = container_id
        artifact["label"] = self._container_label
        artifact["name"] = f"Digital Shadows Data Breach Record {breach_record.id}"
        ...
        artifact["cef"] = {
            "suser": f"{breach_record.username}",
            "sourceHostName": f"{source_host_name}",
            "cs1Label": "Password",
            "cs1": f"{breach_record.password}",
            "cs2Label": "Source Url",
            "cs2": f"{breach.source_url}",
            "cs3Label": "Link to Incident",
            "cs3": link_to_incident,
            "cn1Label": "Digital Shadows Incident Id",
            "cn1": f"{breach.incident_id}",
        }
        return artifact
    ...
    def _ds_to_phantom_severity_transform(self, severity):
        ...
        """
        return {"VERY_HIGH": "high", "HIGH": "high", "MEDIUM": "medium", "LOW": "low", "VERY_LOW": "low", "NONE": "low"}.get(severity)
